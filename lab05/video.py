...
parser.add_argument('--video', action='store_true', help="Save streamed images as video")
...
while True:
    ...
    if magic == 0xBC:
        ...
        if format == 0:
            ...
            if args.video:
                video_writer.write(cv2.cvtColor(bayer_img, cv2.COLOR_GRAY2BGR))
            cv2.waitKey(1)
        else:
            ...
            if args.save:
                cv2.imwrite(f"stream_out/jpeg/img_{count:06d}.jpg", decoded)
            if args.video:
